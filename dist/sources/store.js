    ...
    async fetch(skillName) {
        ...
        try {
            ...
            if (!fs.existsSync(path.join(skillDir, "SKILL.md"))) {
                const available = listDirs(repoRoot).filter((d) => fs.existsSync(path.join(repoRoot, d, "SKILL.md")));
                throw new Error(`Skill "${skillName}" not found in ${storeRepo()}.` +
                    (available.length
