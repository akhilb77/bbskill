 ...
 */
export function resolveSource(ref: string): SkillSource {
  ...
  if (looksLikeRepo) return new GitHubSource();

  if (/^[a-z0-9][a-z0-9-]*$/.test(ref)) return new StoreSource();

  throw new Error(
    `"${ref}" is neither a GitHub repo (owner/repo or URL) nor a valid skill name.`
  );
}
